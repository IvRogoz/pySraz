Create one horizontal sprite strip for the 2D game character `medieval-footman` in animation `walk-down`.
...
Identity lock:
- Do not redesign the character. Only change pose/action for the `walk-down` animation.
...
Animation action: isometric walking down and toward camera cycle, front/top view.

Animation-specific requirements:
- Show a readable alternating step cycle through legs, arms, body tilt, and props only.
...
- Treat the image as 8 equal-width invisible frame slots of 64x64 each.
