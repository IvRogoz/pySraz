Create one horizontal sprite strip for the 2D game character `medieval-footman` in animation `walk-right`.
...
Identity lock:
- Do not redesign the character. Only change pose/action for the `walk-right` animation.
...
Animation action: isometric walking right cycle.
