Create one horizontal sprite strip for the 2D game character `medieval-footman` in animation `walk-left`.
...
Identity lock:
- Do not redesign the character. Only change pose/action for the `walk-left` animation.
...
Animation action: isometric walking left cycle.
