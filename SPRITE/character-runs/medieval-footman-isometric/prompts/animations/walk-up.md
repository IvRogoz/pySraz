Create one horizontal sprite strip for the 2D game character `medieval-footman` in animation `walk-up`.
...
Identity lock:
- Do not redesign the character. Only change pose/action for the `walk-up` animation.
...
Animation action: isometric walking up and away cycle, back/top view.
