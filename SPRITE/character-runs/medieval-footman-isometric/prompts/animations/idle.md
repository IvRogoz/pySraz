Create one horizontal sprite strip for the 2D game character `medieval-footman` in animation `idle`.
...
Identity lock:
- Do not redesign the character. Only change pose/action for the `idle` animation.
...
Animation action: idle stance loop in the same isometric RPG sprite style.
...
- Treat the image as 6 equal-width invisible frame slots of 64x64 each.
