![Screenshot](lua/assets/screenshot.png)
...
This repo contains multiple prototypes; the LÖVE game lives in `lua/` and the instructions below target that folder.
...
- **Flag tiles**: use the second tile variant from `lua/assets/tiles.png`.
- **Holes**: blocked tiles with rock sprites (`Rock1_1.png`, `Rock5_1.png`, `Rock6_1.png`).
- **Trees**: animated obstacles from `lua/assets/Trees_animation.png`.

Trees are placed on at least 20% of valid tiles (when enabled in the menu).

## Animations

- **Idle**: `lua/assets/idlewalk.png` (first 4 frames, per-pawn phase offset).
- **Walk**: `lua/assets/walk.png`.
- **Attack**: `lua/assets/attack.png` (pre-question).
- **Death**: `lua/assets/middleDeath.png`.
- **Sheath/Draw**: `lua/assets/middleDeath.png` rows/cols (post-move draw).

Attack flow is: attack animation -> question(s) -> feedback -> death (if any) -> move -> draw.
...
- Splash video is loaded from `lua/renders/splash.ogv` if present.
- The galaxy shader background uses `lua/shadertoy.mp3` for audio FFT visuals.
